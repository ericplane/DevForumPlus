...
for (const g of GROUPS) {
  ...
  /* `.sort()`, and it is not cosmetic. `readdirSync` returns filesystem order,
   * which NTFS gives back sorted and ext4 does not — so this build was
   * reproducible on Windows, reproducible on Linux, and not reproducible
   * BETWEEN them. See the note on `sortKeys` below. */
 ...
 *
 * `JSON.stringify` emits string keys in insertion order, and these objects are
 * populated while walking a directory. That made `member-types.generated.ts`
 * platform-dependent: identical data, different key order, so a file generated
 * on Windows and regenerated on Linux differed by exactly one (very long) line.
 *
 * That is not a cosmetic diff. release.yml regenerates and refuses to ship when
 * `src/luau/` drifts from the commit — correctly, since a release must be the
 * build that was reviewed — so this made EVERY release fail on CI, for a file
 * nobody had touched. Sorting here is what makes the generator reproducible
 * rather than merely repeatable on one machine.
 */
function sortedJson(value: unknown): string {
  return JSON.stringify(value, (_k, v) => {
    if (v && typeof v === "object" && !Array.isArray(v)) {
      return Object.fromEntries(Object.entries(v as Record<string, unknown>).sort(([a], [b]) =>
        a < b ? -1 : a > b ? 1 : 0));
    }
    return v;
  }, 0);
